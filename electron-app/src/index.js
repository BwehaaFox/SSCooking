...
app.on('ready', async () => {
  ...
  mainWindow.webContents.testtest = 2;
  ...
  ipcMain.on('request-recepie', (event) => {
    // Вместо этого получите ваши настройки из хранилища, например
    const settings = loadConfig(); // { };

    event.reply('get-recepie', settings);
  });

  ipcMain.handle('save-recepies', (event, settings, cb) => {
    // Вместо этого получите ваши настройки из хранилища, например
    const settingss = saveConfig(settings || {}, cb); // { };
  });
});
