...
function getPath(path, modulePrefix, podModulePrefix) {
  ...
  if (
    ...
    path.indexOf(pod_component_file_name) === path.length - pod_component_file_name.length
  ) {
    return path.replace(podModulePrefix + '/', '').replace('/' + pod_component_file_name, '');
  }
...
class TemplateImportProcessor extends BroccoliFilter {
  ...
  getDestFilePath(relativePath) {
    ...
    if (relativePath.indexOf(file_type) === relativePath.length - file_type.length) {
      if (
        (relativePath.indexOf(this.modulePrefix + '/components') === 0 ||
          relativePath.indexOf(this.modulePrefix + '/pods') === 0) &&
        relativePath.indexOf(file_type) == relativePath.length - file_type.length
      ) {
        return relativePath;
      }
    }
